...
import { addFacility, updateFacility } from '../../../services/supabase/facilities';
import { uploadImage } from '../../../services/supabase/storage';

interface FacilityFormProps {
    item: Facility | null; 
    onSave: (savedItem: Facility) => void;
    onCancel: () => void; 
    onDataChange: (newData: Partial<Facility>) => void; // New prop
}

const FacilityForm: React.FC<FacilityFormProps> = ({ item, onSave, onCancel, onDataChange }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-xl w-full max-w-2xl max-h-full overflow-y-auto">
                <h2 className="text-2xl font-bold mb-6 text-gray-800 dark:text-gray-100">{item && item.id ? 'Edit Facility' : 'Create New Facility'}</h2>
