...
import { getExtracurriculars, addExtracurricular, updateExtracurricular, deleteExtracurricular } from '../../services/supabase/extracurriculars'; // Diperbarui untuk menggunakan Supabase
...
const AdminExtracurriculars: React.FC = () => {
    ...
    const handleSave = async (itemData: { name: string; description: string; imageUrl?: string; }) => {
        if (editingItem) {
            await updateExtracurricular(editingItem.id, itemData);
        } else {
            await addExtracurricular(itemData as Omit<Extracurricular, 'id'>);
        }
    ...
    return (
        <div>
            ...
            {isFormVisible && <ExtracurricularForm item={editingItem} onSave={handleSave} onCancel={() => { setIsFormVisible(false); setEditingItem(null); }} />}
